...
exports.getReportsAnalytics = getReportsAnalytics;
exports.getStaffRecentActivities = getStaffRecentActivities;
...
async function getDashboardAnalytics(req, res) {
    try {
        ...
        const orders = (0, db_1.currentDb)().collection('pos_orders');
        const customers = (0, db_1.currentDb)().collection('customers');
        ...
        const recentOrders = await orders
            ...
            .limit(8)
            .toArray();
        const customerIds = Array.from(new Set(recentOrders
            .map((o) => o?.customerId)
            .filter((id) => id && mongodb_1.ObjectId.isValid(String(id)))
            .map((id) => new mongodb_1.ObjectId(String(id)).toHexString()))).map((id) => new mongodb_1.ObjectId(id));
        ...
        const todayOrders = recentOrders.map((o) => {
            ...
            const serviceNames = items.filter((it) => it?.type === 'service').map((it) => String(it?.name || '')).filter(Boolean);
            const staffNames = Array.from(new Set(items.map((it) => String(it?.staff || '')).filter(Boolean)));
            return {
                id: String(o?._id || ''),
                time,
...
async function getReportsAnalytics(req, res) {
    try {
        ...
        const uniqueCustomersCount = Array.isArray(row?.uniqueCustomers)
            ? row.uniqueCustomers.filter((c) => c !== null).length
            : 0;
        ...
        const customerIds = customersAgg.map((c) => c?._id).filter((id) => id && mongodb_1.ObjectId.isValid(String(id)));
        let newCustomers = 0;
        ...
        const todayOrderDetails = todayOrdersRaw.map((o) => {
            ...
            const soldItems = items
                .map((it) => `${String(it?.name || '')}${toNumber(it?.quantity) > 1 ? ` x${toNumber(it?.quantity)}` : ''}`)
                .filter(Boolean)
                .join(', ');
            const staffNames = Array.from(new Set(items.map((it) => String(it?.staff || '')).filter(Boolean)));
            return {
...
async function getStaffRecentActivities(req, res) {
    try {
        const auth = requireAuth(req, res);
        if (!auth)
            return;
