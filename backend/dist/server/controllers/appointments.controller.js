...
async function listAppointments(req, res) {
    try {
        ...
        const appointments = await (0, db_1.currentDb)()
...
async function createAppointment(req, res) {
    try {
        ...
        const existingConflict = await (0, db_1.currentDb)().collection('appointments').findOne({
            date,
            time,
            status: { $ne: 'cancelled' },
            ...(stylistId ? { stylistId } : {}),
        ...
        const result = await (0, db_1.currentDb)().collection('appointments').insertOne(appointment);
...
async function updateAppointment(req, res) {
    try {
        ...
        const current = await (0, db_1.currentDb)().collection('appointments').findOne({ _id: new mongodb_1.ObjectId(id) });
        if (!current) {
            return res.status(404).json({ message: 'Không tìm thấy lịch hẹn.' });
        }
        const stylistId = String(req.body?.stylistId || current.stylistId || '').trim();
        const conflict = await (0, db_1.currentDb)().collection('appointments').findOne({
        ...
        if (req.body?.customerName !== undefined)
            updateDoc.customerName = String(req.body.customerName || '').trim();
        if (req.body?.customerPhone !== undefined)
            updateDoc.customerPhone = String(req.body.customerPhone || '').trim();
        if (req.body?.serviceName !== undefined)
            updateDoc.serviceName = String(req.body.serviceName || '').trim();
        if (req.body?.serviceId !== undefined)
            updateDoc.serviceId = String(req.body.serviceId || '').trim();
        if (req.body?.stylistName !== undefined)
            updateDoc.stylistName = String(req.body.stylistName || '').trim();
        if (req.body?.stylistId !== undefined)
            updateDoc.stylistId = String(req.body.stylistId || '').trim();
        if (req.body?.durationMinutes !== undefined) {
            const duration = Number(req.body.durationMinutes);
            if (Number.isFinite(duration) && duration > 0)
                updateDoc.durationMinutes = duration;
        }
        await (0, db_1.currentDb)().collection('appointments').updateOne({ _id: new mongodb_1.ObjectId(id) }, { $set: updateDoc });
...
async function deleteAppointment(req, res) {
    try {
        ...
        const result = await (0, db_1.currentDb)().collection('appointments').deleteOne({ _id: new mongodb_1.ObjectId(id) });
