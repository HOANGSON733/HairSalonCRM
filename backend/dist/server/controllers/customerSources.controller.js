...
exports.listCustomerSources = listCustomerSources;
exports.createCustomerSource = createCustomerSource;
exports.updateCustomerSource = updateCustomerSource;
exports.deleteCustomerSource = deleteCustomerSource;
const mongodb_1 = require("mongodb");
const customerSourceIconIds_1 = require("../shared/customerSourceIconIds");
...
async function nextSortOrder() {
    const col = (0, db_1.currentDb)().collection('customer_sources');
...
async function listCustomerSources(req, res) {
    try {
        if (!(await requireAuth(req, res)))
            return;
        const items = await (0, db_1.currentDb)()
        ...
        return res.json({
            ...
            sources: items.map((doc) => ({
                ...
                icon: (0, customerSourceIconIds_1.normalizeCustomerSourceIcon)(doc.icon),
            })),
        });
    }
    catch (_error) {
...
async function createCustomerSource(req, res) {
    try {
        if (!(await requireAuth(req, res)))
            return;
        ...
        const icon = (0, customerSourceIconIds_1.normalizeCustomerSourceIcon)(req.body?.icon);
        const now = new Date();
...
async function updateCustomerSource(req, res) {
    try {
        if (!(await requireAuth(req, res)))
            return;
        ...
        const normalizedName = name.toLowerCase();
        const col = (0, db_1.currentDb)().collection('customer_sources');
        const existing = await col.findOne({ _id: new mongodb_1.ObjectId(id) });
        if (!existing) {
        ...
        const icon = req.body?.icon !== undefined
            ? (0, customerSourceIconIds_1.normalizeCustomerSourceIcon)(req.body.icon)
            : (0, customerSourceIconIds_1.normalizeCustomerSourceIcon)(existing.icon);
        await col.updateOne({ _id: new mongodb_1.ObjectId(id) }, {
            $set: {
                name,
                normalizedName,
                icon,
...
async function deleteCustomerSource(req, res) {
    try {
        if (!(await requireAuth(req, res)))
            return;
        const id = String(req.params.id || '').trim();
        ...
        const result = await (0, db_1.currentDb)().collection('customer_sources').deleteOne({ _id: new mongodb_1.ObjectId(id) });
...
//# sourceMappingURL=customerSources.controller.js.map