...
async function createCustomer(req, res) {
    try {
        ...
        const existingCustomer = await (0, db_1.currentDb)().collection('customers').findOne({ phone });
        if (existingCustomer) {
        ...
        const result = await (0, db_1.currentDb)().collection('customers').insertOne(customer);
...
function mapCustomerDoc(customer) {
    return {
...
async function listCustomers(req, res) {
    try {
        ...
        const col = (0, db_1.currentDb)().collection('customers');
        const filter = {};
        if (qRaw) {
