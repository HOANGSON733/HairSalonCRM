...
async function listProductCategories(req, res) {
    try {
        if (!(await requireAuth(req, res)))
            return;
        const categories = await (0, db_1.currentDb)().collection('product_categories').find({}).sort({ createdAt: -1 }).toArray();
        return res.json({
            ok: true,
...
async function createProductCategory(req, res) {
    try {
        ...
        const dup = await (0, db_1.currentDb)().collection('product_categories').findOne({ normalizedName: name.toLowerCase() });
        if (dup)
        ...
        const result = await (0, db_1.currentDb)().collection('product_categories').insertOne(doc);
...
async function updateProductCategory(req, res) {
    try {
        ...
        const current = await (0, db_1.currentDb)().collection('product_categories').findOne({ _id: new mongodb_1.ObjectId(categoryId) });
        if (!current)
        ...
        if (!name)
            return res.status(400).json({ message: 'Vui lòng nhập tên danh mục.' });
        const dup = await (0, db_1.currentDb)().collection('product_categories').findOne({
            normalizedName: name.toLowerCase(),
            _id: { $ne: new mongodb_1.ObjectId(categoryId) },
        });
        if (dup)
            return res.status(409).json({ message: 'Tên danh mục đã tồn tại.' });
        await (0, db_1.currentDb)().collection('product_categories').updateOne({ _id: new mongodb_1.ObjectId(categoryId) }, { $set: { name, normalizedName: name.toLowerCase(), icon, color, description, isVisible, updatedAt: new Date() } });
        if (current.name !== name) {
            await (0, db_1.currentDb)().collection('products').updateMany({ category: current.name }, { $set: { category: name, updatedAt: new Date() } });
        }
...
async function deleteProductCategory(req, res) {
    try {
        ...
        const usedCount = await (0, db_1.currentDb)().collection('products').countDocuments({ category: current.name });
        if (usedCount > 0) {
            if (!replacementCategoryName)
                return res.status(409).json({ message: 'Danh mục đang có sản phẩm. Vui lòng chọn danh mục thay thế.' });
            if (replacementCategoryName === current.name)
                return res.status(400).json({ message: 'Danh mục thay thế phải khác danh mục đang xóa.' });
            const replacement = await (0, db_1.currentDb)().collection('product_categories').findOne({ name: replacementCategoryName });
            if (!replacement)
                return res.status(404).json({ message: 'Không tìm thấy danh mục thay thế.' });
            await (0, db_1.currentDb)().collection('products').updateMany({ category: current.name }, { $set: { category: replacementCategoryName, updatedAt: new Date() } });
        }
        await (0, db_1.currentDb)().collection('product_categories').deleteOne({ _id: new mongodb_1.ObjectId(categoryId) });
