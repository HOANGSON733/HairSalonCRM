...
export async function listServiceCategories(req: Request, res: Response) {
  try {
    ...
    const categories = await currentDb()
...
export async function createServiceCategory(req: Request, res: Response) {
  try {
    ...
    const duplicate = await currentDb().collection('service_categories').findOne({
    ...
    const result = await currentDb().collection('service_categories').insertOne(category);
...
export async function deleteServiceCategory(req: Request, res: Response) {
  try {
    ...
    const category = await currentDb().collection('service_categories').findOne({ _id: new ObjectId(categoryId) });
    if (!category) {
      return res.status(404).json({ message: 'Không tìm thấy danh mục.' });
    }

    const replacementCategoryName = String(req.body?.replacementCategoryName || '').trim();
    const usedCount = await currentDb().collection('services').countDocuments({ category: category.name });
    ...
    if (usedCount > 0) {
      ...
      const replacement = await currentDb().collection('service_categories').findOne({ name: replacementCategoryName });
      if (!replacement) {
    ...
    await currentDb().collection('service_categories').deleteOne({ _id: new ObjectId(categoryId) });
...
export async function updateServiceCategory(req: Request, res: Response) {
  try {
    ...
    const current = await currentDb().collection('service_categories').findOne({ _id: new ObjectId(categoryId) });
    if (!current) {
      return res.status(404).json({ message: 'Không tìm thấy danh mục.' });
    }
    ...
    const duplicated = await currentDb().collection('service_categories').findOne({
      normalizedName: name.toLowerCase(),
    ...
    await currentDb().collection('service_categories').updateOne(
