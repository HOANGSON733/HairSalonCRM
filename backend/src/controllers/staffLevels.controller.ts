  ...
  try {
    ...
    return res.json({
      ...
      staffLevels: items.map((doc) => ({
        ...
        additions: String(doc.additions || '0'),
        deductions: String(doc.deductions || '0'),
...
export async function createStaffLevel(req: Request, res: Response) {
  try {
    ...
    const doc = {
      ...
      additions: String(Number(req.body?.additions || 0)),
      deductions: String(Number(req.body?.deductions || 0)),
